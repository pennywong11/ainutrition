...
// 🟢 已經將會報錯的 dart:html 刪除，統一使用 universal_html
...
class _ReportPageState extends State<ReportPage> {
    ...
    try {
      ...
      if (_customStartDate != null && _customEndDate != null) {
        if (_customStartDate!.year == _customEndDate!.year) {
          dateRangeStr =
              "${_customStartDate!.year}/${_customStartDate!.month}/${_customStartDate!.day} - ${_customEndDate!.month}/${_customEndDate!.day}";
        } else {
          dateRangeStr =
        ...
        dateRangeStr =
            "${_referenceDate!.year}/${_referenceDate!.month}/${_referenceDate!.day}";
      }
      ...
      PdfColor feedbackColor = isWarning
          ? PdfColors.red900
          : PdfColors.green900;
        ...
        if (meal.imagePath.isNotEmpty) {
          ...
          try {
            if (path.startsWith('data:image') ||
              ...
              // 處理 Base64
              final base64String = path.contains(',')
                  ? path.split(',').last
                  : path;
      ...
      if (kIsWeb) {
        ...
        // Android / iOS 原生存檔並觸發分享
        final directory = await getApplicationDocumentsDirectory();
        final file = File('${directory.path}/營養報告.pdf');
        await file.writeAsBytes(bytes);
        await Share.shareXFiles([XFile(file.path)], text: '這是您的飲食營養報告');
      }
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: const Color(0xFF9DC6C2).withOpacity(0.1),
                    borderRadius: BorderRadius.circular(20),
                    border: Border.all(
                      color: const Color(0xFF9DC6C2).withOpacity(0.3),
                    ),
                  ),
                  child: Row(
                    children: [
          ...
          Row(
            children: [
              ...
              Expanded(
                child: _buildSummaryItem(
                  '總重量',
                  '${_reportData?.totalWeight.toStringAsFixed(1) ?? "0.0"} g',
              ...
              Expanded(
                child: _buildSummaryItem(
                  '總熱量',
                  '${_reportData?.totalCalories.toStringAsFixed(0) ?? 0} kcal',
          ...
          Row(
            children: [
              Expanded(
                child: _buildSummaryItem(
                  '蛋白質',
                  '${_reportData?.totalProtein.toStringAsFixed(1) ?? "0.0"} g',
                  Icons.egg,
              ...
              Expanded(
                child: _buildSummaryItem(
                  '碳水',
                  '${_reportData?.totalCarbs.toStringAsFixed(1) ?? "0.0"} g',
                  Icons.water_drop,
              ...
              Expanded(
                child: _buildSummaryItem(
                  '脂質',
                  '${_reportData?.totalFat.toStringAsFixed(1) ?? "0.0"} g',
                  Icons.opacity,
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              _buildAvgColumn(
                '${_reportData?.dailyAverages['protein']?.toStringAsFixed(1) ?? "0.0"} g',
                '蛋白質',
                const Color(0xFF75B5E9),
              ),
              _buildAvgColumn(
                '${_reportData?.dailyAverages['carbs']?.toStringAsFixed(1) ?? "0.0"} g',
                '碳水',
                const Color(0xFF84CACE),
              ),
              _buildAvgColumn(
                '${_reportData?.dailyAverages['fat']?.toStringAsFixed(1) ?? "0.0"} g',
                '脂質',
                const Color(0xFFF5BE76),
              ),
            ],
        ...
        children: [
          ...
          if (_periodFoodList.isNotEmpty) ...[
            ...(() {
                  ...
                  topFoods.sort(
                    (a, b) =>
                        (double.tryParse(b.calories.replaceAll(' 大卡', '')) ?? 0)
                            .compareTo(
                              double.tryParse(
                                    a.calories.replaceAll(' 大卡', ''),
                                  ) ??
                                  0,
                            ),
                  );
  ...
  Widget _buildAvgColumn(String v, String l, Color c) => Expanded(
    child: Column(
      children: [
        Text(
          v,
        ...
        return ClipRRect(
          ...
          child: Image.memory(
            ...
            errorBuilder: (context, error, stackTrace) =>
                _buildImagePlaceholder(mealType),
          ),
        );
      }
      // 3. 判斷是否為網路圖片
  ...
  String _getDateRangeText() {
    ...
    switch (_selectedReportType) {
      case ReportType.weekly:
        return '${_formatDate(_getWeekStartDate(_referenceDate ?? now))} - ${_formatDate(_getWeekEndDate(_referenceDate ?? now))}';
      case ReportType.monthly:
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          border: Border(
            bottom: BorderSide(
              color: isSelected ? const Color(0xFF9DC6C2) : Colors.transparent,
              width: 3,
            ),
          ),
        ),
        child: Row(
          children: [
            Icon(
              icon,
              size: 18,
              color: isSelected ? const Color(0xFF9DC6C2) : Colors.grey,
            ),
            const SizedBox(width: 4),
            Text(
              label,
                      ...
                      child: Column(
                        ...
                        children: [
                          if (isMobile) ...[
                            ...
                            _card3(),
                            const SizedBox(height: 16),
                            _card4(isMobile: true),
                            const SizedBox(height: 16),
                            _card5(isMobile: true),
                          ] else ...[
                            Row(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
